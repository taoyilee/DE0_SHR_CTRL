...
proc set_LEDs {send_data} {
	...
	puts "IR set to 01 (vJTAG shr listen)"
	device_virtual_ir_shift -instance_index 0 -ir_value 01 -no_captured_ir_value
	...
	device_virtual_dr_shift -dr_value $send_data -instance_index 0  -length 451 -no_captured_dr_value
	
	puts "IR set to 10 (sends vkey[0])"
	# Set IR back to 10, which sends vkey[0]
	device_virtual_ir_shift -instance_index 0 -ir_value 10 -no_captured_ir_value
		puts "IR set to 00 (vJTAG armed)"
	# Set IR back to 0, which is bypass mode		device_virtual_ir_shift -instance_index 0 -ir_value 00 -no_captured_ir_value
	closeport
	#return $tdi
}
...
proc IncomingData {sock} {
	...
	if {$data_len != "0"} then {
		...
		set line [string range $line 0 450] 
